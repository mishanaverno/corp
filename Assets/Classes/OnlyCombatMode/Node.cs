	...
	public class Node {
		...
		public void InitShelters(int[,,] incomingMap){ // метод инициирующий, проверяющий соседние клетки, и сохраняющий данные об укрытиях в переменную shelters
			...
			if (this.x < incomingMap.GetLength (1)-1) {  
				if (incomingMap [this.level, this.x + 1, this.y] < 3) 
					this.shelters.bot = incomingMap [this.level, this.x + 1, this.y];
				
			}
			if (this.x > 0) {
				if (incomingMap [this.level, this.x - 1, this.y] < 3)
					this.shelters.top = incomingMap [this.level, this.x - 1, this.y];
			}
			if (this.y < incomingMap.GetLength (2)-1) {
				if (incomingMap [this.level, this.x, this.y + 1] < 3)
					this.shelters.right = incomingMap [this.level, this.x, this.y + 1];
			}
			if (this.y > 0) {
				if (incomingMap [this.level, this.x, this.y - 1] < 3)
					this.shelters.left = incomingMap [this.level, this.x, this.y - 1];
			}
